...
//             send messages via the handler
 ...
 //           sendBTCmessage(BTCommunicator.NO_DELAY, motorRight, right * directionRight, 0);
            ...
            if(left > right +turnMinDifference || left < right -turnMinDifference){
            	if(turnLeft == false){
            		if(left > right){
            			turnLeft = true;
            			sendBTCmessage(BTCommunicator.NO_DELAY, 999, 30, turnSpeed);
            		}
            	}
            	...
            	if(turnRight == false)	
            		if(right > left){
            			turnRight = true;
            			sendBTCmessage(BTCommunicator.NO_DELAY, 999, 30, turnSpeed* -1);
            		}
...
//            
//           if(laptest == false && (left > right +10 || left < right -10)){
//            	laptest = true;
//           	sendBTCmessage(BTCommunicator.NO_DELAY, 999, 30, 0 );
//           }
